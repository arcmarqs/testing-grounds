...
impl SharedData for CalcData {
    ...
    fn serialize_message<W>(w: W, m: &SystemMessage<f32,Arc<Action>,Arc<f32>>) -> Result<()>
        where
            W: Write
    {
    ...
    fn deserialize_message<R>(r: R) -> Result<SystemMessage<f32, Arc<Action>, Arc<f32>>>
        where
            R: Read
    {
...
impl CalcData {
    ...
    fn deserialize_reply_message_capnp(reply: messages_capnp::reply::Reader) -> Result<ReplyMessage<<Self as SharedData>::Reply>> {
        ...
        Ok(ReplyMessage::new(session_id, operation_id, Arc::new(_data)))
    }
    ...
    fn serialize_request_message_capnp(m: &RequestMessage<<Self as SharedData>::Request>, request: &mut messages_capnp::request::Builder) -> Result<()> {
       /* let operation = match m.operation().upgrade() {
        ...
        request.set_data(operation.ref_to_request());
    ...
    fn deserialize_request_message_capnp(request: messages_capnp::request::Reader) -> Result<RequestMessage<<Self as SharedData>::Request>> {
        ...
        let _data = match request.get_data().wrapped_msg(ErrorKind::CommunicationSerialize, "Failed to get data")? {
            ActionRequest::Sqrt => Action::Sqrt,
            ActionRequest::MultiplyByTwo => Action::MultiplyByTwo,
            ActionRequest::NoOp => Action::NoOp,
        };

        Ok(RequestMessage::new(session_id, operation_id, Arc::new(_data)))
    }
